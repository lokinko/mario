...
pub fn analyze(profile: &FinancialProfile, holdings: &[Holding]) -> Vec<RiskFinding> {
    ...
    if profile.monthly_expense > 0.0 && emergency_months < 3.0 {
        findings.push(RiskFinding {
            level: "high".into(),
            title: "应急资金不足".into(),
            detail: format!("当前只能覆盖约 {:.1} 个月支出，市场下跌时可能被迫卖出。", emergency_months),
    ...
    if total > 0.0 {
        if let Some(largest) = holdings.iter().max_by(|a, b| a.market_value.total_cmp(&b.market_value)) {
            let pct = largest.market_value / total * 100.0;
            if pct > 40.0 && largest.asset_class != "现金" {
                findings.push(RiskFinding {
                    level: if pct > 65.0 { "high".into() } else { "medium".into() },
                    title: "组合存在集中暴露".into(),
                    detail: format!("“{}”占组合 {:.1}%，单一判断会显著影响整体结果。", largest.name, pct),
    ...
    if profile.horizon_years <= 2 && holdings.iter().any(|h| matches!(h.asset_class.as_str(), "股票" | "基金")) {
        findings.push(RiskFinding {
            level: "medium".into(),
...
mod tests {
    ...
    fn detects_thin_emergency_fund() {
        let profile = FinancialProfile { monthly_expense: 10_000.0, emergency_fund: 15_000.0, ..Default::default() };
