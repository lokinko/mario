...
import { AlertTriangle, Check, Cloud, Copy, Download, KeyRound, LoaderCircle, LockKeyhole, LogOut, Save, ShieldCheck, Upload, UserRound } from "lucide-react";
import { exportCloudRecoveryKey, getCloudConfig, getCloudStatus, importCloudRecoveryKey, pullCloudSync, pushCloudSync, resendCloudConfirmation, requestCloudPasswordReset, resetCloudPassword, saveCloudConfig, signInCloud, signOutCloud, signUpCloud, verifyCloudPasswordReset } from "../../api";
...
export function CloudSync({ flash, onRestore }: { flash: (message: string) => void; onRestore: () => Promise<void> }) {
  ...
  const initialize = async () => {
    ...
    try {
      ...
      if (request === initialization.current) setError(nextError instanceof Error ? nextError.message : String(nextError));
    } finally {
  ...
  useEffect(() => {
    void initialize();
    return () => { initialization.current += 1; };
  }, []);
  ...
  if (initializing) return <div className="center-screen" role="status"><LoaderCircle className="spin" /><span>正在读取账户与同步状态…</span></div>;
  if (!status) return <div className="center-screen service-error"><AlertTriangle size={28} /><strong>暂时无法读取账户状态</strong><p role="alert">{error}</p><p>本地投资功能仍可使用，你的数据没有被修改。</p><button className="primary" onClick={() => void initialize()}>重新尝试</button></div>;
