...
vi.mock("../../api", () => ({ getDecisions: vi.fn(), getInvestmentRules: vi.fn(), saveDecision: vi.fn(), saveDecisionReview: vi.fn() }));
...
it("preserves edits to an AI draft when rules arrive later", async () => {
  ...
  vi.mocked(api.getInvestmentRules).mockReturnValue(new Promise(r => { resolve = r; }));
  render(<DecisionJournal flash={vi.fn()} seed={{ ...emptyDecision, thesis: "原始草稿" }} clearSeed={vi.fn()} onOpenAnalysis={vi.fn()} />);
...
it("saves a complete manual decision without an account or model", async () => {
  ...
  const draft = { ...emptyDecision, assetName: "观察组合", thesis: "可验证假设", counterThesis: "最强反证", invalidation: "条件不成立", reviewDate: "2026-12-01" };
  render(<DecisionJournal flash={vi.fn()} seed={draft} clearSeed={vi.fn()} onOpenAnalysis={vi.fn()} />);
