...
import { getDecisions, getInvestmentRules, saveDecision, saveDecisionReview } from "../../api";
import type { DecisionEntry, DecisionRecord, DecisionReview, DecisionRuleCheck, InvestmentRule, RuleCheckStatus } from "../../types";
...
export function DecisionJournal({ flash, seed, clearSeed, onOpenAnalysis }: { flash: (s: string) => void; seed: DecisionEntry | null; clearSeed: () => void; onOpenAnalysis: (id: string) => void }) {
  ...
  useEffect(() => { if (seed) setEntry({ ...seed, ruleChecks: checksForRules(rules) }); }, [seed]);
